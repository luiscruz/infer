...
let callback_check_internal_acessors { Callbacks.proc_desc; proc_name; idenv } =
  let do_instr node = function
    | Sil.Call (_, Sil.Const (Sil.Cfun pn), ((_arg1_exp,arg1_typ)::_ as args), loc, _) ->
			...
			then
        let description =
          Printf.sprintf "call to internal accessor method %s" (Procname.to_string pn) in
